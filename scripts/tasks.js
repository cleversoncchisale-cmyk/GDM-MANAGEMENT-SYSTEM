...
(function () {
    ...
    const state = { tasks: [], profiles: [], ministries: [], currentUser: null };
    ...
    function getStoredUser() {
        try { return JSON.parse(localStorage.getItem("gdmCurrentUser") || "null"); }
        catch (_) { return null; }
    }
    ...
    function renderStats(tasks) {
        ...
        set("dueTodayCount",tasks.filter(dueToday).length);
        set("overdueTasksCount",tasks.filter(overdue).length);
        set("upcomingTasksCount",tasks.filter(upcoming).length);
    ...
    function render() {
        ...
        body.innerHTML=tasks.map(t=>{
            const p=progress(t), assigned=profileName(t.assigned_to) || t.assigned_to || "Unassigned";
            const ministry=ministryName(t.ministry_id) || t.ministry || "-";
            const s=STATUS.includes(t.status)?t.status:"pending";
            const pr=PRIORITIES.includes(t.priority)?t.priority:"none";
            return `<tr>
                <td><strong>${esc(t.title || "Untitled task")}</strong><div>${esc(t.description || "")}</div></td>
                <td>${esc(assigned)}</td>
                <td>${esc(ministry)}</td>
                <td>${dateText(t.due_date || t.dueDate)}</td>
                <td><select class="task-status-select" data-task-id="${esc(t.id)}">${STATUS.map(x=>`<option value="${x}" ${x===s?"selected":""}>${statusLabel(x)}</option>`).join("")}</select></td>
                <td><strong>${p}%</strong></td>
                <td><button type="button" class="ghost-btn task-view-btn" data-task-id="${esc(t.id)}">View</button> <button type="button" class="secondary-btn task-edit-btn" data-task-id="${esc(t.id)}">Edit</button> <button type="button" class="ghost-btn task-delete-btn" data-task-id="${esc(t.id)}">Delete</button></td>
            </tr>`;
        }).join("");
    }
    ...
    function createTaskForm() {
        ...
        overlay.innerHTML=`<div class="panel glass-panel" style="position:fixed;inset:50% auto auto 50%;transform:translate(-50%,-50%);width:min(650px,92vw);max-height:90vh;overflow:auto;z-index:9999;padding:25px">
            <div class="section-header"><div><p class="eyebrow">Task Management</p><h2>Assign New Task</h2></div><button type="button" id="closeTaskFormButton" class="ghost-btn">×</button></div>
            <form id="taskForm">
                <label>Task Title<input id="taskTitle" required type="text" placeholder="Enter task title"></label>
                <label>Description<textarea id="taskDescription" rows="4" placeholder="Describe the task"></textarea></label>
                <label>Assign To<select id="taskAssignedTo" required><option value="">Select member</option>${state.profiles.map(p=>`<option value="${esc(p.id)}">${esc(p.full_name||p.display_name||p.email)}</option>`).join("")}</select></label>
                <label>Ministry<select id="taskMinistry"><option value="">No ministry</option>${state.ministries.map(m=>`<option value="${esc(m.id)}">${esc(m.name||m.title)}</option>`).join("")}</select></label>
                <label>Priority<select id="taskPriority">${PRIORITIES.map(x=>`<option value="${x}" ${x==="medium"?"selected":""}>${statusLabel(x)}</option>`).join("")}</select></label>
                <label>Due Date<input id="taskDueDate" type="date"></label>
                <div class="form-actions"><button class="primary-btn" type="submit">Create Task</button><button class="ghost-btn" type="button" id="cancelTaskButton">Cancel</button></div>
            </form></div>`;
        document.body.appendChild(overlay);
        const close=()=>overlay.remove();
        overlay.querySelector("#closeTaskFormButton").onclick=close; overlay.querySelector("#cancelTaskButton").onclick=close;
    ...
    async function viewTask(id) {
        ...
        const assigned=profileName(t.assigned_to)||t.assigned_to||"Unassigned";
    ...
    document.addEventListener("DOMContentLoaded",async()=>{
        ...
        document.addEventListener("click",async e=>{
            const status=e.target.closest(".task-status-select"); if(status)return;
        ...
        document.addEventListener("change",async e=>{ const status=e.target.closest(".task-status-select"); if(!status)return; const value=status.value; await updateTask(status.dataset.taskId,{status:value,progress:value==="completed"?100:undefined}); });
        await Promise.all([loadProfiles(),loadMinistries()]);
        await loadTasks();
    });
